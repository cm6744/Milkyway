...
public class Rect implements Shape
{
    ...
    {
        float rad = (float) Mth.toRadians(degree);
    ...
    public boolean interacts(Rect rect)
    {
        ...
        if(VecMth.dot(centers, axisY) > h2 + rw2 * xy + rh2 * yy) {
            return false;
        }
        if(VecMth.dot(centers, rect.axisX) > rw2 + w2 * xx + h2 * xy) {
            return false;
        }
        if(VecMth.dot(centers, rect.axisY) > rh2 + w2 * yx + h2 * yy) {
            return false;
        }
        return true;
    }
