...
import cm.milkywaytool.physics.Flat;
...
public class AreaStatic implements Area
{
    ...
    private AreaStatic pushTexture(BufferTex buf)
    {
        buffer = buf;
        return this;
    }
    ...
    private AreaStatic vertex(double u1, double v1, double u2, double v2)
    {
        double w = u2 - u1;
        double h = v2 - v1;
        uv.loc(u1, v1);
    ...
    private AreaStatic vertex01(double u1, double v1, double u2, double v2)
    {
        return vertex(u1 * fw(), v1 * fh(), u2 * fw(), v2 * fh());
    }

    private AreaStatic dim01(double u1, double v1, double uw, double vh)
    {
        return vertex01(u1, v1, u1 + uw, v1 + vh);
    }

    private AreaStatic dim(double u1, double v1, double uw, double vh)
    {
        return vertex(u1, v1, u1 + uw, v1 + vh);
    }

    private AreaStatic full()
    {
        dim01(0, 0, 1, 1);
        return this;
    }
